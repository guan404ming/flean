...
def roundAndPack {spec : BinarySpec} (mode : RoundingMode) (isNeg : Bool)
  ...
  if finalExp ≥ maxExp then
    ...
    let resSign := if isNeg then BitVec.ofNat 1 1 else BitVec.ofNat 1 0
    let resM := BitVec.ofNat p (finalSig % 2^p)
    let resE := BitVec.ofNat spec.expWidth finalExp.toNat
    let inexact := (g || r || s || roundUp)
    let underflow := (rawExp < 1) && inexact
    { value := FloatBits.fromFields resSign resE resM,
